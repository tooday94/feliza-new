...
function Menu1CategoryList() {
    ...
    if (isLoading) {
        return (
            <div className="max-w-[1280px] w-full mx-auto px-4 py-8 overflow-x-auto">
                <div className="flex gap-4 sm:gap-6 w-max">
                    {skeletonItems.map((_, index) => (
                        <div
                            key={index}
                            className="w-[45vw] sm:w-[220px] h-[140px] sm:h-[180px] bg-gray-200 rounded-lg animate-pulse"
                        />
                    ))}
                </div>
            </div>
