...
function ProductDetail() {
    ...
    const { data } = useGetList(endpoints.products.getProductById + id, {});
    const { data: productVariants, isLoading: loadvar } = useGetList(
        endpoints.products.searchProduct + data?.referenceNumber
    );

    const { mutate } = useCreate(endpoints.cart.addCartItem, endpoints.cart.getCart);
    ...
    const addToCart = () => {
        ...
        if (!selectedColorVariant) {
            toast.error(i18n.language === "uz" ? "Rangni tanlang" : "Выберите цвет", { autoClose: 1000 });
            return;
        }
        ...
        if (!selectedSizeVariant) {
            toast.error(i18n.language === "uz" ? "O‘lchamni tanlang" : "Выберите размер", { autoClose: 1000 });
            return;
        }

        if (selectedSizeVariant.quantity === 0) {
            toast.warning(i18n.language === "uz" ? "Mahsulot qolmagan" : "Нет в наличии", { autoClose: 1000 });
            return;
        }
        ...
        mutate(
            ...
            {
                onSuccess: () => {
                    toast.success(i18n.language === "uz" ? "Savatchaga qo‘shildi" : "Добавлено в корзину", { autoClose: 1000 });
                    setCount(1);
                },
                onError: () => {
                    toast.error(i18n.language === "uz" ? "Xatolik yuz berdi" : "Ошибка", { autoClose: 1000 });
                },
            }
        );
    };
    ...
    const addToFavorites = () => {
        ...
        addtofavorites(
            {
                customerId: userID,
                productId: selectedColorVariant?.id,
            },
            {
                onSuccess: () => {
                    toast.success(i18n.language === "uz" ? "Sevimlilarga qo‘shildi" : "Добавлено в избранное", { autoClose: 1000 });
                },
                onError: () => {
                    toast.error(i18n.language === "uz" ? "Xatolik yuz berdi" : "Ошибка", { autoClose: 1000 });
                },
    ...
    if (loadvar) {
        return <div className="text-center py-10 text-gray-500 text-lg">Yuklanmoqda...</div>;
    }

    return (
        <div className="font-tenor md:px-6 mt-10">
                ...
                <div className="flex flex-wrap gap-2">
                    {productVariants?.map((item, index) => (
                        <div
                            ...
                            className={`w-[80px] h-[100px] border cursor-pointer overflow-hidden ${selectedColorIndex === index ? "border-black" : "border-gray-300"
                                }`}
                        >
                            <img
            ...
            <div className="mt-7">
                ...
                <div className="flex flex-wrap gap-3 mt-3">
                    {productVariants?.[selectedColorIndex]?.productSizeVariantList?.map((item, index) => (
                        <button
                            ...
                            className={`border px-4 py-2 cursor-pointer transition ${selectedSize === item.size
                                ? "bg-black text-white"
                ...
                {selectedSize && (
                    <p className="mt-2 text-gray-600">
                        ...
                        {
                            productVariants[selectedColorIndex]?.productSizeVariantList?.find(
            ...
            <div className="flex items-center gap-4 mt-8">
                <div className="flex items-center border rounded-md overflow-hidden">
                    <FaMinus
                        ...
                        className={`px-3 py-2 cursor-pointer ${count <= 1 ? "opacity-30 pointer-events-none" : ""
                            }`}
                    />
                    ...
                    <FaPlus
                        ...
                        className={`px-3 py-2 cursor-pointer ${count >=
                            (productVariants[selectedColorIndex]?.productSizeVariantList?.find(
                                (item) => item.size === selectedSize
                            )?.quantity ?? 0)
                ...
                <button
                    onClick={addToCart}
                    className="w-full h-12 border border-black hover:bg-black hover:text-white flex items-center justify-center gap-2 transition"
                >
                ...
                <button
                    ...
                    className="p-3 border border-gray-300 hover:bg-red-500 hover:text-white transition rounded-md"
                >
                    <FaRegHeart />
                </button>
            </div>
